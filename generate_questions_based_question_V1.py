...
questions_topics = load_json("./answers_generated_final_p1.json")
# Informacion: ```{context}```
...
Tu tarea consiste en generar 8 preguntas únicas para cada una de las siguientes preguntas, las preguntas generdas deben basarse respectivamente en cada pregunta original y ser relevantes para estudiantes de la Facultad de Ciencias de la Univeridad Nacional de Ingenieria. Es importante que las preguntas busquen obtener más detalles o información específica sobre tema o puntos relevantes mencionados en cada pregunta original, sin desviarse del mismo. Además, es esencial que las preguntas mantengan coherencia con los conceptos y contexto establecido en la información delimitada por tres comillas invertidas.

Lista de preguntas:
{list_questions}
Información: ```{context}```

Presenta las 8 preguntas únicas basadas en cada pregunta original de la siguiente manera:
...
...
8. [Aquí la pregunta 8 basada la primer pregunta orignal]
...
Tu tarea consiste en generar 8 preguntas únicas para cada una de las siguientes preguntas, las preguntas generdas deben basarse respectivamente en cada pregunta original y ser relevantes para estudiantes de la Facultad de Ciencias de la Univeridad Nacional de Ingenieria. 
Es importante que las preguntas busquen obtener más detalles o información específica sobre el tema o puntos relevantes mencionados en cada pregunta original, sin desviarse del mismo. Además, es esencial que las preguntas mantengan coherencia con los conceptos y contexto establecido en la información delimitada por tres comillas invertidas.
...
def extracts_questions_basades(texto):
    ...
    for bloque in bloques_preguntas :
        ...
        preguntas = re.findall(patron_pregunta, bloque, re.DOTALL)
...
for questions_about_topic in questions_topics[:1]:
    ...
    for i in range(2):
        ...
        generate_questions_based_questions_7(b_questions, context)

    #questions = [qt["question"] for qt in questions_about_topic["questions"]]
   
    

    #for qa in question_answer_pairs[0:3]:
