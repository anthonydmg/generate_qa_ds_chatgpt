...
def get_prompt_gen_factoid_questions(reglamento, num_questions = 20):
    print(f"\nNumero de preguntas a crear {num_questions}")
    prompt = f"""
Tu tarea es generar al menos {num_questions} pares de preguntas y respuestas, basadandote en la información proporcionada sobre reglamentos académicos de la Facultad de Ciencias de la UNI (delimitada por tres comillas invertidas). 
...
Tambien al generar cada respuesta asegurate de cumplir cada uno de los siguientes criterios:
    - Indica claramente que no posees información para responder a la pregunta si esta no tiene respuesta basada en la información proporcionada.
    - Para las preguntas que sí tengan respuesta, justifica detalladamente las razones de la respuesta basada en la información proporcionada.
    - Proporciona información complementaria, respaldada en la información proporcionada, 
    - Suministra detalles complementarios pertinentes basados en la información proporciona para enriquecer las respuestas a las preguntas.
    - Evita mencionar numerales de artículos que no mencionen al documento al que pertenecen en las respuestas; en su lugar, puedes referencia directamente al reglamento de matrícula de la Facultad de Ciencias de la UNI.
    - El reglamento solo debe referenciarse de manera completa como el reglamento de matrícula de la Facultad de Ciencias de la UNI dentro de las respuestas.

Finalmente, presenta los {num_questions} pares de preguntas y respuestas de la siguiente manera:
1. pregunta...
Respuesta: ....
2. pregunta...
Respuesta: ...
...
class QuestionGenerator:
        ...
        for file_text in documents[3:4]:
            ...
            try:    
                ...
                for qt in self.questions_types:
                    ...
                    qas = self.generate_questions_answers(text, qt, num_questions = num_questions)
                    ...
                    questions_generated.append({"document": file_text, "type":  QuestionType(qt).name.lower(), "qas_pairs": qas})
                    time.sleep(10)
                ...
                save_json("./", "qas_generated", questions_generated)
                raise Exception(e)
    ...
    def generate_questions_answers(self, text, question_type, num_questions = 30):
        ...
        qas = self.extract_questions_answers_from_response_regex(response)
        print("Total de preguntas generadas:", len(qas))
        if abs(num_questions - len(qas)) > 3:
            ...
            print("response:", response)
            print("Size output: " , count_tokens(encoding ,response))
            next_qas = self.extract_questions_answers_from_response_regex(response)
            print("Total de preguntas generadas:", len(next_qas))
            qas  = qas + next_qas
        print("Total de preguntas extraidas:", len(qas))
