...
mensaje del usuario: ```{query}```
    ...
    def reformulated_contextual_query(self, query, history_chat_messages):
        history_chat = self.format_text_history_chat(history_chat_messages)
        prompt_identify_reform = f"""
Dado el último mensaje del usuario dado en el contexto de Facultad de Ciencias de La Universidad Nacional de Ingeniería de Peru enviado a un asistente familiarizado con las normativas académicas de la universidad, analiza e indica si la ultima pregunta del usuario podría entender total por si sola, sin necesidad de tener acceso a tiene acceso al historial previo de la conversación. Solo si la pregunta no es entendible por si sola reformula la pregunta de manera que incluya todo el contexto necesario para que pueda entenderse en su totalidad sin necesidad del historial del chat. No respondas el mensaje, solo reformúlal. 

Has tu análisis para identificar si la pregunta es entendible basándote en los siguientes criterios:
...
Realiza el análisis de manera minuciosa basándote en los criterios e indica de la siguiente manera si es que la ultima pregunta del usuario proporcionada se entiende sin necesidad del historial y porporciona la pregunta reformulado solo si es necesario:
 
- Análisis: ...
- La última pregunta del usuario se entiende sin necesidad del historial del chat: Sí o No
- Reformulación: <<Pregunta reformulada>>

Último mensaje del usuario: {query}

Historio previo de la conversacion: <<{history_chat}>>
"""
...
if __name__ == "__main__":
    ...
    save_dir = "./conversational_faq/openline-reformulated"
    path_file = "faq-reformulated/data/faq_23_reformulated.json"
    filename = "faq_23_reformulated.json"
