...
class QuestionGenerator:
    ...
    def run(self):
        ...
        subtopic_selected = self.topics_data[27:28]

        questions_generated = []
        
        progress_bar = tqdm(range(len(subtopic_selected)), desc= "Sub Temas")
        total_questions = 0
        for subtopic in subtopic_selected:
                ...
                for qt in self.questions_types:
                    ...
                    if len(questions_to_mod) > 0:
                        ...
                        for i_q_mod, q_to_mod in enumerate(questions_to_mod):
                            print("\norignal:",questions[q_to_mod["index"]]["question"])
                            print("\nmod:",questions_mod[i_q_mod])
                            print()
                            questions[q_to_mod["index"]]["question"] = questions_mod[i_q_mod]
                    ...
                    questions_generated.append({
                        ...
                        "general_topic": subtopic["topic"],
                        "general_subtopic": subtopic["subtopic"],
                        "source_context": subtopic["source"], 
                        "questions": questions,
    ...
    def generate_questions_answers(self, text, question_type, num_questions = 30):
        ...
        questions = self.extract_questions_topic_from_response_regex(response)
        #print("Total de preguntas generadas:", len(questions))
        if abs(num_questions - len(questions)) > 1:
            print("\nFaltan preguntas\n")
            content = prompt + "\n" + response
